...
public class VasoimmolationPower extends AbstractPower{
    ...
    public void atEndOfTurn(final boolean isPlayer) {
        if (!p.hasPower(IntoxicationPower.POWER_ID))
            return;
        flash();
        int damage = p.getPower(IntoxicationPower.POWER_ID).amount * (this.amount / 100);
        if (!upgraded)
            addToBot(new DamageAction(this.p, new DamageInfo(p, damage, DamageType.NORMAL), AttackEffect.FIRE));
        addToBot(new DamageAllEnemiesAction(this.p, damage, DamageType.NORMAL, AttackEffect.FIRE));
    }
